...
const createBook = async (req, res) => {
    ...
    try {
        const { title, description, author, category, price, discount, image, userId } = req.body;
        const bookId = uuidv4();

        const result = await session.run(
            `
            MATCH (u:User {id: $userId})
            CREATE (b:Book {id: $bookId, title: $title, description: $description, author: $author, category: $category, price: $price, discount: $discount, image: $image, postedBy: $userId})
            MERGE (u)-[:POSTED]->(b)
            RETURN b
            `,
...
const editBook = async (req, res) => {
    ...
    try {
        ...
        const updateResult = await session.run(
            'MATCH (b:Book {id: $id}) SET b += {title: $title, description: $description, author: $author, category: $category, price: $price, discount: $discount, image: $image} RETURN b',
